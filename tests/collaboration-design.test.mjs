...
test('collaboration form prepares an email and hands it off to the user mail service', () => {
  ...
  assert.ok(js.includes('mailto:'), 'native email-app fallback is missing');
  assert.ok(js.includes('data-contact-submit'), 'submit hook is missing');
  assert.ok(js.includes('showInvalidField'), 'visible invalid-field handling is missing');
  assert.ok(!js.includes("fetch('/api/contact'"), 'direct backend submission must be disabled');
});
