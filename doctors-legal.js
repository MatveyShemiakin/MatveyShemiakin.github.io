(function(){
  ...
  function createClinicalNotice(){const article=document.querySelector('.article');if(!article||article.querySelector('.md-prof-clinical'))return;const notice=document.createElement('section');notice.className='md-prof-notice md-prof-clinical';notice.setAttribute('aria-labelledby','md-prof-clinical-title');notice.innerHTML=html`<div class="md-prof-notice__inner"><div><span class="md-prof-eyebrow">${T.clinicalKicker}</span><h2 id="md-prof-clinical-title">${T.clinicalTitle}</h2></div><div class="md-prof-notice__copy"><p>${T.clinicalP1}</p><p>${T.clinicalP2}</p>${professionalLink()}</div></div>`;article.prepend(notice);}
