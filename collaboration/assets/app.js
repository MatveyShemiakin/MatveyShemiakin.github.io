(() => {
  ...
  function preparedMessage() {
    ...
    const bodyText = isEn
      ? [`Name: ${name}`, org ? `Organization / role: ${org}` : '', `Reply email: ${reply}`, `Topic: ${topic}`, '', message, '', 'Prepared on matveyshemyakin.ru/collaboration/'].filter(Boolean).join('\n')
      : [`Имя: ${name}`, org ? `Организация / должность: ${org}` : '', `E-mail для ответа: ${reply}`, `Тема: ${topic}`, '', message, '', 'Подготовлено на matveyshemyakin.ru/collaboration/'].filter(Boolean).join('\n');
    return { subject, bodyText };
  }

  function gmailComposeUrl(subject, bodyText) {
  ...
  function nativeMailUrl(subject, bodyText) {
    return `mailto:${recipientAddress()}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(bodyText)}`;
  }

  function renderHandoffFallback(gmailUrl, mailtoUrl) {
    if (!status) return;
    status.className = 'form-status success';
    status.replaceChildren();
    const text = document.createElement('span');
    text.textContent = isEn ? 'Email prepared. If Gmail did not open: ' : 'Письмо подготовлено. Если Gmail не открылся: ';
    const gmail = document.createElement('a');
    gmail.href = gmailUrl;
    gmail.target = '_blank';
    gmail.rel = 'noopener noreferrer';
    gmail.textContent = isEn ? 'open Gmail' : 'открыть Gmail';
    const separator = document.createTextNode(isEn ? ' or ' : ' или ');
    const native = document.createElement('a');
    native.href = mailtoUrl;
    native.textContent = isEn ? 'email app' : 'почтовое приложение';
    status.append(text, gmail, separator, native);
  }
  ...
  form?.addEventListener('submit', e => {
    ...
    const { subject, bodyText } = preparedMessage();
    const gmailUrl = gmailComposeUrl(subject, bodyText);
    const mailtoUrl = nativeMailUrl(subject, bodyText);
    renderHandoffFallback(gmailUrl, mailtoUrl);
