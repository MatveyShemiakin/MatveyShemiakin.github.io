...
## 8. Контроль перед активацией

- [ ] создан сервисный аккаунт `clinical-navigator-ai`;
- [ ] назначена роль `ai.languageModels.user`;
- [ ] создан API-ключ со scope `yc.ai.languageModels.execute`;
- [ ] ключ сохранён только в переменной Cloud Function;
- [ ] установлен `YANDEX_FOLDER_ID`;
- [ ] установлен `YANDEX_MODEL=yandexgpt/latest`;
- [ ] таймаут увеличен до 90 секунд;
- [ ] память увеличена до 256 МБ;
- [ ] после сохранения preview показывает `Провайдер: yandex`;
- [ ] контрольный HLA-B27-кейс не вызывает повторных вопросов.
